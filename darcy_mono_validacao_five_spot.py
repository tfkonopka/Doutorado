...
def perm_2k(perm_m, perm_v):
    ...
    bc2 = DirichletBC(W.sub(0), Constant((3.0e-6, 0.0)), boundaries, 2)
    bc3 = DirichletBC(W.sub(0), Constant((0.0, -3.0e-6)), boundaries, 3)
    ...
    # L = inner(f, v) * dx
    L = (
        inner(f, v) * dx - pout * dot(v, n) * ds(6) - pout * dot(v, n) * ds(7)
    )  # - pin * dot(v, n) * ds(1)
    ...
    u_file = File(
        "/home/tfk/Desktop/results/Brinkman/Brinkman_Biphase/vugg_sintetico/Mono_Brinkman/canal_vertica_perm1/darcy_mono_u.pvd"
    )
    u_file << u1
    p_file = File(
        "/home/tfk/Desktop/results/Brinkman/Brinkman_Biphase/vugg_sintetico/Mono_Brinkman/canal_vertica_perm1/darcy_mono__p.pvd"
    )
    p_file << p1
