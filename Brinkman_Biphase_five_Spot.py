...
def BrinkmanIMPES(Nx, _folder_base, mu_w, mu_o, perm_darcy, dt, pin, pout):
    ...
    A_in = float(assemble(1 * ds(2)) + assemble(1 * ds(3)))
    A_out = float(assemble(1 * ds(6)) + assemble(1 * ds(7)))
    ...
    while step < 1e6:
        ...
        Qdotw_vector.append(
            ...
            + assemble(F(s0, mu_rel, noo_proj, nww_proj) * dot(u_, n) * ds(7))
        ...
        pin = (
            assemble(p_ * ds(2)) + assemble(p_ * ds(3))
        ) / A_in  # pressão média na entrada
        ...
        uin = assemble(dot(u_, n) * ds(2)) + assemble(dot(u_, n) * ds(3))
        uout = assemble(dot(u_, n) * ds(6)) + assemble(dot(u_, n) * ds(7))
